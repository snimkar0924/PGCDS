...
library(fGarch)
library(quantmod)
da<-read.table("C:\\Users\\rajani\\Desktop\\ch4data\\m-intcsp7309.txt",header=T)
head(da)
intc=log(da$intc+1)
...
archTest(y,12)
fx<-read.table("C:\\Users\\rajani\\Desktop\\ch4data\\d-useu9910.txt",header=T)
head(fx)
fxeu<-log(fx$rate)
eu<-diff(fxeu)
Box.test(eu,lag=12)
...
tdx<-(c(1:444)/12)+1973
par(mfcol=c(3,1))
plot(tdx,resi,xlab="year",ylab="Stand.resid")
acf(resi,lag=20)
pacf(resi^2,lag=20)
...
summary(m4)
v1<-volatility(m4)
resi1<-residuals(m4,standardize=T)
vol<-ts(v1,freq=12,start=c(1973,11))
res1<-ts(resi1,frequency=12,start=c(1973,1))
par(mfcol=c(2,2))
acf(resi1,lag=24)
pacf(resi1,lag=24)
acf(resi1^2,lag=24)
...
m5<-garchFit(~1+garch(1,1),data=intc,cond.dist="std",trace=F)
summary(m5)
m6<-garchFit(~1+garch(1,1),data=intc,cond.dist="sstd",trace=F)
summary(m6)
v2<-volatility(m5)
v3<-volatility(m6)
par(mfcol=c(3,1))
plot(tdx,v1,xlab="year", ylab="volatility",type="l",ylim=c(0.06,0.3),col="red")
title(main="(A) Gaussian")

plot(tdx,v2,xlab="year", ylab="volatility",type="l",ylim=c(0.06,0.3),col="green")
title(main="(B) Student's t")
plot(tdx,v3,xlab="year", ylab="volatility",type="l",ylim=c(0.06,0.3),col="blue")
title(main="(C) Skew Student's t")
